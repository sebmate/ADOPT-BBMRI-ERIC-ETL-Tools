...
public class ETLHelper {
	...
	public static void main(String[] args) throws SQLException, IOException {
		...
		System.out.println("Welcome to the BBMRI-ERIC ETL Helper Utility 1.04!\n");
		...
		FileUploader fileUploader = new FileUploader(MDRPipeConfiguration.getDataFileName(),
				MDRPipeConfiguration.getMdrSourceFileName(), MDRPipeConfiguration.getMdrTargetFileName(),
				MDRPipeConfiguration.getMappingFileName());
		fileUploader.uploadFiles();

		// Start the matching of the elements from the source namespace to the
		// target namespace
		...
		System.out.println("");
		System.out.println(
				"If you experience any problems with this program, please contact christian.knell@fau.de or sebastian.mate@fau.de.");
	...
	private static void readCommandLine(String[] args) {
		...
		Option configurationId = new Option("configId", "configurationId", true,
				"ID of the configuration that should be used");
	...
	public static void showSummaryOfETL() throws SQLException {
		int originalNumberOfDataRecords = ETLHelper.getElementMatcher().getDatabase().getDatabaseTableData()
				.countValuesInColumn(DatabaseTableData.DATA_VALUE);
		int numberOfDataRecordsWithMappingRuleAndSuccessfulTransformation = ETLHelper.getElementTransformer()
				.getDataTypeTransformations().countNumberOfTranslationRulesBySuccess("OK");
		double percent = ((double) numberOfDataRecordsWithMappingRuleAndSuccessfulTransformation
				/ (double) originalNumberOfDataRecords) * 100.0;
		System.out.println("");
		System.out.println("=== Final summary about the ETL process ===");
		System.out.println("");
		System.out.println("In total, " + ETLHelper.round(percent, 2)
				+ "% of the input data records could be mapped and transformed.");
	}
